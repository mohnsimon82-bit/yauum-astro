...
(function(){
  ...
  var prodToggle=document.querySelector(".nav-prod-toggle");
  var prodPanel=document.getElementById("production-panel");
  if(prodToggle&&prodPanel){
    var prodWrap=prodToggle.parentNode;
    var setProd=function(open){
      prodToggle.setAttribute("aria-expanded",String(open));
      if(open){prodWrap.setAttribute("data-open","");}else{prodWrap.removeAttribute("data-open");}
    };
    prodToggle.addEventListener("click",function(){
      setProd(prodToggle.getAttribute("aria-expanded")!=="true");
    });
    document.addEventListener("click",function(e){
      if(!prodPanel.contains(e.target)&&e.target!==prodToggle)setProd(false);
    });
    document.addEventListener("keydown",function(e){
      if(e.key==="Escape")setProd(false);
    });
  }
